...
const CardSwap = ({
  ...
  useEffect(() => {
    // Respect reduced motion — render the stack statically.
    ...
    return () => {
      if (intervalRef.current) clearInterval(intervalRef.current);
      tlRef.current?.kill();
      if (pauseOnHover && node && pause && resume) {
  ...
  return (
    <div ref={container} className={cn(styles.cardSwapContainer, className)} style={{ width, height }}>
      {rendered}
    </div>
  );
};
